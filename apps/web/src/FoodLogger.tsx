...
import { createFoodEntry, lookupBarcode, mealEntries, recentMeals, repeatMealEntries, scaleFood, searchFoods, type FoodDefinition, type FoodEntry, type MealType, type SavedMeal } from './foodLog.js';
...
import { MealPhotoLogger } from './MealPhotoLogger.js';
...
export function FoodLogger({ date, entries, favoriteFoodIds, savedMeals, foodDataClient, choicePriority, onChange, onPreferencesChange, onClose }: Props) {
  ...
  const recentMealSuggestions = useMemo(() => recentMeals(entries, date, meal), [date, entries, meal]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void foodDataClient.search(query.trim()).then((foods) => { if (active) { setRemoteFoods(foods); setSearchState('idle'); } }).catch(() => { if (active) { setRemoteFoods([]); setSearchState('unavailable'); } });
    }, 250);
    return () => { active = false; window.clearTimeout(timer); };
  }, [foodDataClient, query]);

  function addCatalogFood(foodId: string) { const food = results.find((candidate) => candidate.id === foodId); if (food) onChange([...entries, createFoodEntry(date, meal, scaleFood(food, quantity))]); }
  ...
  function repeatRecentMeal(recentMeal: ReturnType<typeof recentMeals>[number]) { onChange([...entries, ...repeatMealEntries(recentMeal, date, meal)]); }
  ...
  return <div className="workout-backdrop" onMouseDown={onClose}><section ref={dialogRef} className="food-logger" role="dialog" aria-modal="true" aria-labelledby="food-logger-title" tabIndex={-1} onMouseDown={(event) => event.stopPropagation()}>
    ...
    {photoLoggerOpen && foodDataClient && <MealPhotoLogger date={date} meal={meal} client={foodDataClient} onAdd={(photoEntries) => onChange([...entries, ...photoEntries])} onClose={() => setPhotoLoggerOpen(false)} />}
  </section></div>;
}
