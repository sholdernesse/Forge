        ...
        <section className="content-grid">
          ...
          <article className="panel training-history-panel">
            ...
            {selectedTraining && <section className="training-history-detail" aria-labelledby="training-history-detail-title">
              ...
              {selectedTrainingComparison && <div className="history-comparison"><div><span className="section-label">VS PREVIOUS MATCH · {new Date(`${selectedTrainingComparison.previousDate}T12:00:00Z`).toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' })}</span><b>Change since your last {selectedTraining.title}</b></div><span><small>Duration</small><b>{selectedTrainingComparison.duration.delta > 0 ? '+' : ''}{selectedTrainingComparison.duration.delta} min</b><em>{selectedTrainingComparison.duration.previous} → {selectedTrainingComparison.duration.current}</em></span><span><small>Completed sets</small><b>{selectedTrainingComparison.completedSets.delta > 0 ? '+' : ''}{selectedTrainingComparison.completedSets.delta}</b><em>{selectedTrainingComparison.completedSets.previous} → {selectedTrainingComparison.completedSets.current}</em></span><span><small>Reported effort</small><b>{selectedTrainingComparison.effort ? `${selectedTrainingComparison.effort.delta > 0 ? '+' : ''}${selectedTrainingComparison.effort.delta}` : 'Not comparable'}</b><em>{selectedTrainingComparison.effort ? `${selectedTrainingComparison.effort.previous} → ${selectedTrainingComparison.effort.current}` : 'Both sessions need a rating'}</em></span>{selectedTrainingComparison.exercises.length > 0 && <section className="history-exercise-comparison"><header><b>Movement continuity</b><small>Previous → current completed sets</small></header>{selectedTrainingComparison.exercises.map((exercise) => <span key={exercise.exerciseId}><b>{exercise.name}</b><em>{exercise.previousSets} → {exercise.currentSets}</em><strong className={exercise.delta === 0 ? 'same' : ''}>{exercise.delta > 0 ? '+' : ''}{exercise.delta}</strong></span>)}</section>}</div>}
