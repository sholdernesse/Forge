      ...
      <main>
        ...
        <section className="content-grid">
          <article className="panel workout-panel" id="training">
            ...
            {deload.active && workout.planType !== 'recovery' && <div className="deload-alert"><ShieldAlert size={18} /><span><b>{planDeload ? 'Consolidation week active' : 'Deload protection active'}</b><small>Sets reduced {Math.round((1 - deload.volumeMultiplier) * 100)}% and load reduced {Math.round((1 - deload.loadMultiplier) * 100)}%{planDeload ? ' · planned after three building weeks' : ` · fatigue score ${deload.fatigueScore}`}</small></span></div>}
