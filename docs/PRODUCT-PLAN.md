...
**Current checkpoint:** Sprint 4.57 on PR #2
...
- ~~Camera barcode capture with manual fallback.~~ Native secure-camera scanning delivered in Sprint 4.55; Sprint 4.57 makes HTTPS and browser capability failures explicit instead of allowing a silent no-op. Broader decoder fallback remains dependent on an approved client library or native app shell.
